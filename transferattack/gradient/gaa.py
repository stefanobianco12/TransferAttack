...
class GAA(Attack):
    ...
    def __init__(self, model_name, epsilon=16/255, alpha=1.6/255, beta=3.0, lambda_upper=0.2, upper_bound=1.6/255,num_neighbor=20, epoch=10, decay=1., targeted=False, 
                random_start=False, norm='linfty', loss='crossentropy', device=None, attack='GAA', **kwargs):
    ...
    def get_averaged_gradient(self, data, delta, label, **kwargs):
        ...
        for _ in range(self.num_neighbor):
            ...
            x_next = self.transform(x_near + self.upper_bound*(g_1 / (torch.abs(g_1).mean(dim=(1,2,3), keepdim=True))))
            ...
            averaged_gradient += (1-self.lambda_upper)*g_1 + (1+self.lambda_upper)*g_2
